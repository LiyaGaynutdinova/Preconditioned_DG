...
%  nodes = nodes + randn(size(nodes))/20;
...
% [x,flag,relres,iterG]  = cgs(A,B,toler,max_steps); % max_steps
...
casPDG = toc;
% tic;
% [x,flag,relres,iterDG]  = cgs(ADG,BDG,toler,5); % max_steps
% casDG = toc;
% iteraceDGandPDG = [iterDG,iterPDG]
% timeDGandPDG = [casDG,casPDG]
itertaionsPDG = iterPDG
return
title = strcat('resultsDG_Ex31_Test1_',num2str(NNN(1)),'.mat');
save(title,'conditionPAbound','iteraceGaPG','conditionPPP_ADG_bound',...
    'iteraceDGandPDG','timeDGandPDG' );
...
e_contrast = 1; % = 1,2,3,4
y = sin(5*x1.^2+x2.^2*5)+1+10^(-e_contrast);
